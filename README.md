...
독서 기록 플랫폼 Chak을 위한 UI 라이브러리입니다 ✨
<br/>
...
🟢 chak-blocks는 `plain 컴포넌트`와 `context 컴포넌트`로 구분됩니다.
<br/>
`context 컴포넌트`를 사용할 때는 파일 상단에 `'use client'`를 작성하여 클라이언트 컴포넌트임을 명시해주세요.

#### Plain 컴포넌트

`Toast`, `Accordion`을 제외한 모든 컴포넌트는 `plain 컴포넌트`입니다.
<br/>
서버 컴포넌트, 클라이언트 컴포넌트 모두 자유롭게 사용 가능합니다.
<br/>
(브라우저 렌더링 후 javascript를 통한 동적 액션이 예상된다면 클라이언트 컴포넌트로 사용해주세요.)
...
React의 Context API를 사용하는 컴포넌트입니다. `Toast`와 `Accordion`이 포함됩니다.
<br/>
클라이언트 컴포넌트로만 사용이 가능합니다.
...
<br/>
HTML 엘리먼트와 스타일을 조합하여 다양한 Typography를 만들 수 있습니다.
...
| `... button attributes` |                                                                       | button 엘리먼트의 속성을 지원합니다. |                                     |
...
| `... SVG Attributes` |                                                                                                                                                         | SVG 엘리먼트의 속성을 지원합니다. |                                   |
...
| `children`            | `React.ReaFactNode `                   | Card의 자식 요소입니다.            | `* Required`                       |
